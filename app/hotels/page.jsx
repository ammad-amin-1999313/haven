                ...
                <div className='border-b mb-10 '>
                    <div className="mb-12 max-w-2xl  ">
                        ...
                        <p className="text-gray-500 text-sm leading-relaxed">
                            From secluded mountain cabins to vibrant city suites, find the perfect space for your next chapter.
                        </p>
                    </div>
                </div>
                ...
                <div className="flex flex-col lg:flex-row gap-10 ">
                    ...
                    <aside className="w-full lg:w-64 space-y-8">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <span className="text-xs text-gray-500">Up to ${priceRange}</span>
                            </div>
                            <input
                                type="range"
                                min="100"
                            ...
                            <div className="space-y-3">
                                {['Pool', 'Spa', 'Wi-Fi', 'Parking', 'Restaurant', 'Gym'].map((item) => (
                                    <label key={item} className="flex items-center gap-3 cursor-pointer group">
                                        <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-[#2D5A4C] focus:ring-[#2D5A4C]" />
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex justify-between items-center mb-6">
                            <p className="text-sm text-gray-500">Showing {hotels.length} properties</p>

                            {/* Sort Dropdown */}
                            <div className="relative">
                                <button
                                    onClick={() => setIsSortOpen(!isSortOpen)}
                                    className="flex items-center gap-2 text-sm font-medium border border-gray-200 px-4 py-2 rounded-lg bg-white hover:bg-gray-50 transition-all min-w-[170px] justify-between shadow-sm"
                                >
                                    <span>{selectedSort}</span>
                                    <ChevronDown
                                        size={16}
                                        className={`transition-transform duration-500 ease-in-out ${isSortOpen ? 'rotate-180' : 'rotate-0'
                                            }`}
                                    />
                                </button>
                                            ...
                                            {sortOptions.map((option) => (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${selectedSort === option
                                                        ? "bg-[#2D5A4C]/10 text-[#2D5A4C] font-bold"
