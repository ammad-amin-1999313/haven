...
const AddHotel = () => {
  ...
  const validate = () => {
    ...
    if (!roomTypes.length)
      next.roomTypes = "At least one room type is required";
  ...
  const inputClasses = (error) =>
    `w-full px-4 py-3 rounded-xl border bg-[#FDFDFD] focus:outline-none focus:ring-2 focus:ring-[#2D5A4C]/20 transition ${
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        ...
        <div>
          ...
          >
            ...
            {/* 2. Dynamic Upload Box */}
            {images.length < MAX_IMAGES && (
              <div
                onClick={() => fileInputRef.current.click()}
                className={`
        rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center cursor-pointer transition-all group
        ${
          images.length === 0
            ? "w-full h-64 bg-gray-50"
            : "aspect-square w-full"
              ...
              >
                ...
                />
                <span className="text-sm text-gray-400 mt-2 group-hover:text-blue-500 font-medium">
                  {images.length === 0
                ...
                {images.length === 0 && (
                  <p className="text-xs text-gray-400 mt-1">
                    Up to {MAX_IMAGES} photos • PNG, JPG
                  </p>
                )}
              </div>
            )}
        ...
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {amenities.map((a) => (
              <span
                key={a}
                className="px-3 py-1 rounded-full bg-[#2D5A4C]/10 text-[#2D5A4C] text-[10px] font-bold uppercase flex items-center gap-2"
              >
                {a}{" "}
                <X
                  size={12}
                  className="cursor-pointer"
                  onClick={() => setAmenities((p) => p.filter((x) => x !== a))}
                />
              </span>
            ))}
          </div>
        ...
        <div className="space-y-6">
          ...
          {roomTypes.map((rt, idx) => (
            <div
              key={idx}
              className="p-6 border rounded-2xl bg-gray-50 space-y-4"
            >
